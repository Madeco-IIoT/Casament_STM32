...
void simon_game_over(void){
	char game_str[STR_GAME_OVER];
	uint16_t best_game = 0;
	ssd1306_Fill(Black);
	ssd1306_SetCursor(16,10);
	...
	char sc1=comptador_simon+'0';
	ssd1306_WriteChar(sc1, Font_11x18,0x01);

	best_game = HAL_RTCEx_BKUPRead(&hrtc, RTC_BKP_DR2);
	memset(game_str, '\0', STR_GAME_OVER);
	sprintf(game_str, "Best score : %i", best_game);
	if(best_game < comptador_simon) HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR2, comptador_simon);
	ssd1306_SetCursor(10,50);
	ssd1306_WriteString(game_str, Font_7x10, White);
	ssd1306_UpdateScreen();
	HAL_Delay(5000);
}
